...
modded class SCR_BaseGameMode {
   ...
   override void OnGameStart() {
      ...
      m_iAirdropEventId = bus.RegisterCustomEvent(LES_AirdropEventIds.MOD_ID, LES_AirdropEventIds.EVENT_NAME);
   ...
   protected void LES_FireDemoAirdrop() {
      ...
      int id = LES_EventBus.GetInstance().FindCustomEvent(LES_AirdropEventIds.MOD_ID, LES_AirdropEventIds.EVENT_NAME);
      if (id == -1)
         return;
   ...
   protected void LES_OnAirdropCalled(LES_EventPayload payload) {
      Print("[LES Example] Airdrop called at: " + payload.m_sContext + " | cargo=" + payload.GetTag("cargo") + " amount=" + payload.GetTag("amount") + " (" + payload.GetTagCount() + " tags)");
   }
}
